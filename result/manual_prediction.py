...
temperature_scaler = load('../prediction/scalers/temperature_scaler.bin')
percipitation_scaler = load('../prediction/scalers/percipitation_scaler.bin')
...
def get_number_input_in_range(msg, range_min, range_max):
    while True:
        ...
        if val < range_min or val > range_max:
            print(
                f"Die Zahl ist nicht im Wertebereich von {range_min} bis {range_max}")
            continue
...
if PREDICTION_MODE == "c":
    ...
    while input_station_yn not in "yn":
        input_station_yn = input(
            "Eigene Wetterstation auswählen Standard ist 617 (y/n): ")
...
SELECTED_MONTH = get_number_input_in_range("Monat (1 bis 12): ", 1, 12)
print("Sonntag = 1")
print("Montag = 2")
print("Dienstag = 3")
print("Mittwoch = 4")
...
SELECTED_HOUR = get_number_input_in_range(
    "2-Stunden-intervall 0 = 0-2, 1 = 2-4, ... (0 bis 11): ", 0, 11)
SELECTED_TEMPERATURE = get_number_input(
    f"Temperatur in °C (Durchschnitt: {temperature_scaler.mean_}): ")
SELECTED_TEMPERATURE = temperature_scaler.transform(
    [[SELECTED_TEMPERATURE]])[0][0]
# print(f'Transformed temperature: {SELECTED_TEMPERATURE}')
SELECTED_percipation = get_number_input(
    f"Niederschlag in mm (Durchschnitt: {percipitation_scaler.mean_}): ")
SELECTED_percipation = temperature_scaler.transform(
    [[SELECTED_percipation]])[0][0]
# print(f'Transformed percipitation: {SELECTED_percipation}')
...
rf_pred = rf_model.predict([[SELECTED_MONTH, SELECTED_DAY, SELECTED_HOUR,
                             SELECTED_TEMPERATURE, SELECTED_percipation, SELECTED_USAGE]])

nn_pred = nn_model.predict([[SELECTED_MONTH, SELECTED_DAY, SELECTED_HOUR,
                             SELECTED_TEMPERATURE, SELECTED_percipation, SELECTED_USAGE]])

if PREDICTION_MODE == "c":
    accident_scaler = load(
        f'../prediction/scalers/accident_scaler_{SELECTED_STATION}.bin')
    rf_pred = accident_scaler.inverse_transform([rf_pred])
    nn_pred = accident_scaler.inverse_transform(nn_pred)
